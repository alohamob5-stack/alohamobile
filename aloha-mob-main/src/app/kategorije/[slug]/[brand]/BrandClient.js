...
{!isMobile && mounted && (
            shopSession ? (
                          <>
                <button onClick={() => router.push('/moje-porudzbine')} style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', padding: '0.5rem 0.9rem', background: '#1a1a1a', color: '#fff', border: '1px solid #333', borderRadius: '6px', cursor: 'pointer', fontWeight: '600', fontSize: '0.85rem', whiteSpace: 'nowrap' }}>Istorija</button>
                <button onClick={() => setLogoutConfirmOpen(true)} style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', padding: '0.5rem 0.9rem', background: '#1a1a1a', color: '#fff', border: '1px solid #333', borderRadius: '6px', cursor: 'pointer', fontWeight: '600', fontSize: '0.85rem', whiteSpace: 'nowrap' }}>{shopSession.type === 'shop' ? shopSession.locationName : shopSession.name} Odjava</button>
  </>
            ) : (
                          <button onClick={() => setAuthModalOpen(true)} style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', padding: '0.5rem 0.9rem', background: '#1a1a1a', color: '#fff', border: '1px solid #333', borderRadius: '6px', cursor: 'pointer', fontWeight: '600', fontSize: '0.85rem', whiteSpace: 'nowrap' }}>Prijava</button>
                        )
                                  )}
        ...
        <main style={{ minWidth: 0 }}>
          ...
          {isSpecialMode && (selectedModel || selectedTypeFilter) && (
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.4rem', marginBottom: '0.75rem' }}>
              ...
              {selectedTypeFilter && <span onClick={() => setSelectedTypeFilter(null)} style={{ background: 'var(--red-light)', color: 'var(--red)', padding: '0.3rem 0.75rem', borderRadius: '20px', fontSize: '0.85rem', fontWeight: '600', cursor: 'pointer' }}>{selectedTypeFilter} ✕</span>}
            </div>
          )}
          {!isSpecialMode && selectedSubcat && (
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.4rem', marginBottom: '0.75rem' }}>
              <span onClick={() => setSelectedSubcat(null)} style={{ background: 'var(--red-light)', color: 'var(--red)', padding: '0.3rem 0.75rem', borderRadius: '20px', fontSize: '0.85rem', fontWeight: '600', cursor: 'pointer' }}>{subcats.find(s => s.id === selectedSubcat)?.name} ✕</span>
            </div>
          )}
  ...
  return (
    <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', overflow: 'hidden', boxShadow: 'var(--shadow)', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ padding: '0.5rem 0.6rem', borderTop: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: '0.3rem', background: 'var(--bg3)' }}>
        <button onClick={e => { e.stopPropagation(); setQty(q => Math.max(1, q - 1)); }} style={{ width: btnSize, height: btnSize, border: '1px solid var(--border)', borderRadius: '6px', cursor: 'pointer', background: '#fff', fontWeight: 'bold', fontSize: btnFont, flexShrink: 0 }}>−</button>
        <span style={{ fontWeight: '700', fontSize: btnFont, minWidth: '20px', textAlign: 'center' }}>{qty}</span>
        <button onClick={e => { e.stopPropagation(); setQty(q => q + 1); }} style={{ width: btnSize, height: btnSize, border: '1px solid var(--border)', borderRadius: '6px', cursor: 'pointer', background: '#fff', fontWeight: 'bold', fontSize: btnFont, flexShrink: 0 }}>+</button>
