    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)', fontFamily: 'sans-serif' }}>
      <header style={{ background: 'var(--black)', padding: '0 1rem' }}>
        <div style={{ maxWidth: '900px', margin: '0 auto', display: 'flex', alignItems: 'center', gap: '0.75rem', height: '56px' }}>
          <div onClick={() => router.push('/')} style={{ fontSize: '1.4rem', fontWeight: '900', color: '#fff', cursor: 'pointer' }}>
            ALOHA<span style={{ color: 'var(--red)' }}>MOB</span>
          </div>
          <button onClick={() => setLogoutConfirmOpen(true)} style={{ marginLeft: 'auto', background: 'none', border: '1px solid #333', color: '#fff', padding: '0.4rem 0.9rem', borderRadius: '6px', cursor: 'pointer', fontSize: '0.85rem' }}>
            Odjava
          </button>
          <button onClick={() => router.push('/')} style={{ background: 'none', border: '1px solid #333', color: '#fff', padding: '0.4rem 0.9rem', borderRadius: '6px', cursor: 'pointer', fontSize: '0.85rem' }}>
            ← Početna
          </button>
        </div>
      </header>
                ...
                return (
                  <div key={o.id} style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', marginTop: '0.6rem', boxShadow: 'var(--shadow)', overflow: 'hidden' }}>
                    <div onClick={() => setOpenOrderId(isOrderOpen ? null : o.id)}
                      ...
                      <span style={{ alignSelf: 'flex-start', padding: '0.3rem 0.75rem', borderRadius: '20px', fontSize: '0.8rem', fontWeight: '700', color: statusColors[o.status], background: statusBg[o.status] }}>
