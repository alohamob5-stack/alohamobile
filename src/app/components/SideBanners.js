  ...
  const Banner = ({ banner }) => (
    <div
      ...
      style={{
        ...
        background: banner?.bg || 'linear-gradient(160deg, var(--black), var(--lagoon))',
        overflow: 'hidden',
      }}
    >
      {banner?.image_url ? (
        ...
        <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '0.75rem', padding: '1rem', textAlign: 'center' }}>
          <span style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '42px', height: '42px', borderRadius: '13px', background: 'rgba(255,255,255,0.12)', color: 'var(--red)' }}><Palmtree size={22} /></span>
          <div className="font-head" style={{ color: 'var(--red)', fontWeight: '700', fontSize: '0.85rem' }}>ALOHA<span style={{ color: '#fff' }}>MOB</span></div>
          ...
          <div style={{ background: 'var(--red)', color: '#fff', padding: '0.4rem 0.7rem', borderRadius: 'var(--pill)', fontWeight: '700', fontSize: '0.72rem' }}>🔥 -30%</div>
        </div>
      )}
    </div>
  );
