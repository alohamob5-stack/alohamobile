...
import { CheckCircle2, Truck, Search, User, ShoppingCart, Home as HomeIcon, Package, Trash2, Banknote, CreditCard, LogOut, ImageOff, Flame, Sparkles, Palmtree } from 'lucide-react';
...
function PretragaContent() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)', fontFamily: 'sans-serif', paddingBottom: isMobile ? '65px' : '0' }}>
      ...
      <div style={{ background: 'var(--lagoon)', color: '#fff', textAlign: 'center', padding: '0.4rem 1rem', fontSize: '0.82rem', fontWeight: '600', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.4rem' }}>
      ...
      <header style={{ background: '#fff', padding: '0 1rem', position: 'sticky', top: 0, zIndex: 100, boxShadow: '0 2px 16px rgba(15,30,61,0.08)', borderBottom: '1px solid var(--border)' }}>
        <div style={{ maxWidth: '1700px', margin: '0 auto', display: 'flex', alignItems: 'center', gap: '0.75rem', height: '56px' }}>
          <div onClick={() => router.push('/')} style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', fontFamily: 'var(--font-head)', fontSize: '1.4rem', fontWeight: '700', color: 'var(--black)', cursor: 'pointer', whiteSpace: 'nowrap', flexShrink: 0 }}>
            <span style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '34px', height: '34px', borderRadius: '11px', background: 'linear-gradient(135deg, var(--black), var(--lagoon))', color: '#fff', flexShrink: 0 }}><Palmtree size={18} /></span>
          ...
          <div style={{ flex: 1, position: 'relative' }}>
            <input ref={searchInputRef} placeholder="Pretraži proizvode..." value={searchVal} onChange={e => setSearchVal(e.target.value)}
              ...
              style={{ width: '100%', padding: '0.5rem 0.75rem 0.5rem 2.2rem', borderRadius: 'var(--pill)', border: '1.5px solid var(--border)', outline: 'none', fontSize: '0.9rem', background: 'var(--bg3)', color: 'var(--text)' }} />
            <Search size={15} style={{ position: 'absolute', left: '0.6rem', top: '50%', transform: 'translateY(-50%)', color: 'var(--muted)' }} />
            {searchVal && (
              <button onClick={() => router.push(`/pretraga?q=${encodeURIComponent(searchVal.trim())}`)}
                style={{ position: 'absolute', right: '0.5rem', top: '50%', transform: 'translateY(-50%)', background: 'var(--red)', border: 'none', color: '#fff', borderRadius: 'var(--pill)', padding: '0.2rem 0.7rem', cursor: 'pointer', fontSize: '0.8rem', fontWeight: '700' }}>
                Traži
              </button>
            )}
          </div>
          {!isMobile && mounted && (
            shopSession ? (
              <button onClick={() => setLogoutConfirmOpen(true)}
                style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', padding: '0.5rem 0.9rem', background: '#fff', color: 'var(--black)', border: '1.5px solid var(--border)', borderRadius: 'var(--pill)', cursor: 'pointer', fontWeight: '600', fontSize: '0.85rem', whiteSpace: 'nowrap' }}>
          ...
          {!isMobile && (
            <button onClick={() => setDrawerOpen(true)}
              style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.5rem 1.2rem', background: 'var(--red)', color: '#fff', border: 'none', borderRadius: 'var(--pill)', cursor: 'pointer', fontWeight: '700', fontSize: '0.9rem', whiteSpace: 'nowrap', boxShadow: '0 4px 14px rgba(239,138,28,0.35)' }}>
          ...
          <>
            ...
            {activeFiltersCount > 0 && (
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.4rem', marginBottom: '0.75rem' }}>
                {selectedBrand && <span onClick={() => setSelectedBrand(null)} style={{ background: 'var(--red-light)', color: 'var(--red)', padding: '0.3rem 0.75rem', borderRadius: 'var(--pill)', fontSize: '0.85rem', fontWeight: '600', cursor: 'pointer' }}>{availableBrands.find(b => b.id === selectedBrand)?.name} ✕</span>}
                {selectedProtection && <span onClick={() => setSelectedProtection(null)} style={{ background: 'var(--red-light)', color: 'var(--red)', padding: '0.3rem 0.75rem', borderRadius: 'var(--pill)', fontSize: '0.85rem', fontWeight: '600', cursor: 'pointer' }}>{selectedProtection} ✕</span>}
                {(priceMin > minPrice || priceMax < maxPrice) && <span onClick={() => { setPriceMin(minPrice); setPriceMax(maxPrice); }} style={{ background: 'var(--red-light)', color: 'var(--red)', padding: '0.3rem 0.75rem', borderRadius: 'var(--pill)', fontSize: '0.85rem', fontWeight: '600', cursor: 'pointer' }}>{priceMin.toLocaleString('sr-RS')}–{priceMax.toLocaleString('sr-RS')} RSD ✕</span>}
              </div>
            )}
            ...
            {!loading && filtered.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '4rem 1rem', color: 'var(--muted)' }}>
                ...
                {activeFiltersCount > 0 && <button onClick={clearAllFilters} style={{ marginTop: '1rem', padding: '0.6rem 1.5rem', background: 'var(--red)', color: '#fff', border: 'none', borderRadius: 'var(--pill)', cursor: 'pointer', fontWeight: '700' }}>Ukloni filtere</button>}
              </div>
            ) : (
                ...
                {filtered.map(p => (
                  <div key={p.id} style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', overflow: 'hidden', boxShadow: 'var(--shadow)', display: 'flex', flexDirection: 'column' }}>
                    ...
                    <div style={{ padding: '0.5rem 0.75rem', borderTop: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: '0.4rem', background: 'var(--bg3)' }}>
                      <button onClick={() => setQtys(qq => ({ ...qq, [p.id]: Math.max(1, (qq[p.id]||1) - 1) }))} style={{ width: '28px', height: '28px', border: '1px solid var(--border)', borderRadius: 'var(--pill)', cursor: 'pointer', background: '#fff', fontWeight: 'bold', flexShrink: 0 }}>−</button>
                      <span style={{ fontWeight: '700', fontSize: '0.9rem', minWidth: '20px', textAlign: 'center' }}>{qtys[p.id] || 1}</span>
                      <button onClick={() => setQtys(qq => ({ ...qq, [p.id]: (qq[p.id]||1) + 1 }))} style={{ width: '28px', height: '28px', border: '1px solid var(--border)', borderRadius: 'var(--pill)', cursor: 'pointer', background: '#fff', fontWeight: 'bold', flexShrink: 0 }}>+</button>
                      <button onClick={() => handleAddBtn(p)} style={{ flex: 1, padding: '0.4rem', background: added[p.id] ? '#16a34a' : 'var(--red)', color: '#fff', border: 'none', borderRadius: 'var(--pill)', cursor: 'pointer', fontWeight: '700', fontSize: '0.8rem', transition: 'background 0.2s' }}>
                        {added[p.id] ? '✓' : '+ Dodaj'}
                      </button>
                    </div>
                  </div>
