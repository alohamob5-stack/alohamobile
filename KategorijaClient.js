          ...
          {!isMobile && mounted && (
            shopSession ? (
              <button onClick={() => setLogoutConfirmOpen(true)}
                style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', padding: '0.55rem 0.9rem', background: '#fff', color: 'var(--black)', border: '1.5px solid var(--border)', borderRadius: 'var(--pill)', cursor: 'pointer', fontWeight: '600', fontSize: '0.85rem', whiteSpace: 'nowrap' }}>
        ...
        <main style={{ minWidth: 0 }}>
          ...
          {activeFiltersCount > 0 && (
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.4rem', marginBottom: '0.75rem' }}>
              {selectedBrand && <span onClick={() => setSelectedBrand(null)} style={{ background: 'var(--red-light)', color: 'var(--red)', padding: '0.3rem 0.75rem', borderRadius: 'var(--pill)', fontSize: '0.85rem', fontWeight: '600', cursor: 'pointer' }}>{allProds.find(p => p.brand_id === selectedBrand)?.brands?.name} ✕</span>}
              {selectedSubcat && <span onClick={() => setSelectedSubcat(null)} style={{ background: 'var(--red-light)', color: 'var(--red)', padding: '0.3rem 0.75rem', borderRadius: 'var(--pill)', fontSize: '0.85rem', fontWeight: '600', cursor: 'pointer' }}>{allProds.find(p => p.subcategory_id === selectedSubcat)?.subcategories?.name} ✕</span>}
              ...
              {selectedConn1 && <span onClick={() => setSelectedConn1(null)} style={{ background: 'var(--red-light)', color: 'var(--red)', padding: '0.3rem 0.75rem', borderRadius: 'var(--pill)', fontSize: '0.85rem', fontWeight: '600', cursor: 'pointer' }}>K1: {selectedConn1} ✕</span>}
              {selectedConn2 && <span onClick={() => setSelectedConn2(null)} style={{ background: 'var(--red-light)', color: 'var(--red)', padding: '0.3rem 0.75rem', borderRadius: 'var(--pill)', fontSize: '0.85rem', fontWeight: '600', cursor: 'pointer' }}>K2: {selectedConn2} ✕</span>}
              ...
              {selectedMaskType && <span onClick={() => setSelectedMaskType(null)} style={{ background: 'var(--red-light)', color: 'var(--red)', padding: '0.3rem 0.75rem', borderRadius: 'var(--pill)', fontSize: '0.85rem', fontWeight: '600', cursor: 'pointer' }}>{selectedMaskType} ✕</span>}
              {selectedWatt && <span onClick={() => setSelectedWatt(null)} style={{ background: 'var(--red-light)', color: 'var(--red)', padding: '0.3rem 0.75rem', borderRadius: 'var(--pill)', fontSize: '0.85rem', fontWeight: '600', cursor: 'pointer' }}>{selectedWatt}W ✕</span>}
            </div>
          )}
          {isMobile ? (
    ...
    <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', overflow: 'hidden', boxShadow: 'var(--shadow)', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ padding: '0.5rem 0.6rem', borderTop: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: '0.3rem', background: 'var(--bg3)' }}>
        <button onClick={e => { e.stopPropagation(); setQty(q => Math.max(1, q-1)); }}
          style={{ width: btnSize, height: btnSize, border: '1px solid var(--border)', borderRadius: 'var(--pill)', cursor: 'pointer', background: '#fff', fontWeight: 'bold', fontSize: btnFont, flexShrink: 0 }}>−</button>
